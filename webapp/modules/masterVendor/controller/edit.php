...
$tool->abrir_transaccion();
    ...
    ///clave
    $clave1 = $tool->simple_db("select pass from tbl_vendedor where id = $id and cuenta_id = $cuenta ");
    $clave2 = $tool->getvar("clave",$_POST);
    if($clave1!=$clave2) $_POST['r9pass'] = md5($clave2);



    $tool->update_data("r","9","tbl_vendedor",$_POST,"id = $id  and cuenta_id = $cuenta ");

    if(!isset($_POST['r9activo']))
        $tool->query("update tbl_vendedor set activo = 0 where id = $id  and cuenta_id = $cuenta ");
